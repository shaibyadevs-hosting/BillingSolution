	...
	try {
		...
		if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
			return NextResponse.json(
				{
					error:
						"SUPABASE_SERVICE_ROLE_KEY is not configured. This is required for license creation. Please add it to your environment variables.",
		...
		try {
			...
			console.log(
				"[API /license/seed] No authentication (this is allowed for license seeding)"
			);
		}
		...
		const isEmergencyLicense =
			isEmergency === true ||
			macAddress?.trim().toUpperCase() === "EMERGENCY" ||
			macAddress?.trim().toUpperCase() === "ANY";
		...
		if (isEmergencyLicense) {
			...
			if (!macAddress || !macAddress.trim()) {
				return NextResponse.json(
					{
						error:
							"MAC address is required (or set isEmergency: true for emergency license)",
					},
					{ status: 400 }
				);
			}
			...
			if (!/^[0-9A-F]{12}$/.test(normalizedMac)) {
				return NextResponse.json(
					{
						error:
							"Invalid MAC address format. Expected format: XX:XX:XX:XX:XX:XX or XXXXXXXXXXXX",
					},
					{ status: 400 }
				);
			}
		...
		if (queryError) {
			...
			return NextResponse.json(
				{
					error:
						"Failed to query licenses. Please check your Supabase configuration.",
					details:
		...
		const expiresOn = new Date(
			Date.now() + finalExpiresInDays * 24 * 60 * 60 * 1000
		).toISOString();
		...
		try {
			...
			console.error(
				"[API /license/seed] License data that failed:",
				licenseData
			);
			...
			if (dbError.message) {
				if (
					...
					errorMessage =
						"Permission denied. Please ensure SUPABASE_SERVICE_ROLE_KEY is configured in your environment variables.";
				} else if (
					dbError.message.includes("relation") &&
					dbError.message.includes("does not exist")
				) {
					errorMessage =
						"Licenses table not found. Please run the database migration script.";
				} else {
	...
	return NextResponse.json(
		{
			...
			message:
				"Use POST method to seed a license. Example: POST /api/license/seed with body { macAddress: 'AA:BB:CC:DD:EE:FF' }",
		},
		{ status: 405 }
	);
}
